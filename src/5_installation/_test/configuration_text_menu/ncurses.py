...
class CursesMenu(object):
    ...
    def prompt_selection(self, parent=None):
        ...
        while input_key != ENTER_KEY:
            ...
            if self.selected_option == option_count:
                self.screen.addstr(5 + option_count, 4, "{}".format(lastoption), self.hilite_color)
            else:
                self.screen.addstr(5 + option_count, 4, "{}".format(lastoption), self.normal_color)
    ...
    def my_raw_input(self, r, c, prompt_string):
        curses.echo() 
        self.screen.addstr(r, c, prompt_string)
        self.screen.refresh()
        input = self.screen.getstr(r + 1, c, 20)
        return input
